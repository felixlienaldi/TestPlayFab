...
//Client -> dapet Exp -> taro exp di server -> server processing level -> 
handlers.UpdatePostMatchesStatistics = function(args){
    let succeedPost = true;
    let Experience = args.Exp;
    let isWinning = args.isWinning;
    let HeroesName = args.HeroName;
    let MaxExperience = 0;
    ...
    try{
        ...
        let PlayerStatistic = server.GetPlayerStatistics({
            PlayFabId : currentPlayerId,
        }).Statistics;

        for(let i=0;i<PlayerStatistic.length;i++){
            if(PlayerStatistic[i].StatisticName == "Max Experience"){
                MaxExperience = PlayerStatistic[i].Value;
            }else if(PlayerStatistic[i].StatisticName == "Current Experience"){
                CurrentExperience = PlayerStatistic[i].Value;
            }else if(PlayerStatistic[i].StatisticName == "Level"){
                Level = PlayerStatistic[i].Value;
            }else if(PlayerStatistic[i].StatisticName == "Win"){
                WinCount = PlayerStatistic[i].Value;
            }else if(PlayerStatistic[i].StatisticName == "Lose"){
                LoseCount = PlayerStatistic[i].Value;
            }else if(PlayerStatistic[i].StatisticName == "TotalMatches"){
                MatchCount = PlayerStatistic[i].Value;
            }else if(PlayerStatistic[i].StatisticName == HeroesName){
                HeroesExp = PlayerStatistic[i].Value;
            }
        }
        ...
        if(CurrentExperience >= MaxExperience){
            MaxExperience += 250;
            Level++;
            //MaxExperience nambahnya juga dari sini tinggal rumusnya aja..
            //CurrentExperience = 0  tinggal rumusnya aja..
        }

        server.UpdatePlayerStatistics({
            PlayFabId: currentPlayerId,
            Statistics:[{
                "StatisticName": "Max Experience",
                "Value": MaxExperience
            },{
                "StatisticName": "Current Experience",
                ...
                "StatisticName": "TotalMatches",
                "Value": MatchCount
            },{
                "StatisticName": HeroesName,
                "Value": HeroesExp
            }]
        });
    ...
    return {UpdatePostMatchesStatistics : succeedPost}
};

handlers.AddFriendRequest = function(args){
    let succeed = true;
    let FriendssPlayFabID;
    let ListFriends;
    try{
        server.AddFriend({
            FriendTitleDisplayName : args.FriendDisplayName,
            PlayFabId : currentPlayerId,
        });

        try{
            ListFriends = server.GetFriendsList({
        ...
        try{
            ListFriends.forEach(Friend => {
                if(Friend.Tags == null){
                    FriendssPlayFabID = Friend.FriendPlayFabId;
                }
            });
        }catch(err){
            log.error(err + "Kedua");
        ...
        server.SetFriendTags({
            FriendPlayFabId : FriendssPlayFabID,
            PlayFabId : currentPlayerId,
            Tags : "PendingSent",
        });
    ...
    try{
        server.AddFriend({
            ...
            PlayFabId : FriendssPlayFabID,
        });
        
        server.SetFriendTags({
           FriendPlayFabId : currentPlayerId,
           PlayFabId : FriendssPlayFabID,
           Tags : "PendingConfirmed",
        });
    }catch (err) {
        succeed = false;
    }
    
    return { messageValue: succeed };
};

handlers.ConfirmFriendRequest = function(args){
    let succeed = true;
    try{
        server.SetFriendTags({
           FriendPlayFabId : currentPlayerId,
    ...
    return { HasBeenConfirmed :  succeed};
};
